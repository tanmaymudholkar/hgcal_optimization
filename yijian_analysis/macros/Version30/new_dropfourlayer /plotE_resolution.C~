  ...
  for (unsigned int eta_counter = 2; eta_counter <= 2; eta_counter++) {
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over rechits
	  ...
	  double weighted_energy = energy*weights[layer]/tanh(eta_values[eta_counter]);
          ...
          for(int dropcounter=0;dropcounter<4;dropcounter++){
             if(layer==droplayers[dropcounter]+1) weighted_energy = energy*(weights[layer]+weights[layer-1])/tanh(eta_values[eta_counter]);
             if(layer==droplayers[dropcounter]) weighted_energy = 0;
          }
	  if(sumE[layer]>0 && weighted_energy!=0 && abs(int(posx)/10-(int)(centerx[layer])/10)<=truncationwidth && abs((int)posy/10-(int)(centery[layer])/10)<=truncationwidth) // truncate around energy center 
    ...
    TF1 *function_to_fit = new TF1("f_to_fit",resolutions_fit,0.0001,1000,3);
    function_to_fit->SetParameters(0.015,0.1,1);
    function_to_fit->SetParNames("const_offset","coeff_sqrt","coeff_add");
    ...
    double bestfit_add = function_to_fit->GetParameter(2);
    double bestfit_add_error = function_to_fit->GetParError(2);
    resolutions_versus_energy->SetTitle(start_title_resolutions + Form("%6.5f +/- %6.5f) + (%4.3f +/- %4.3f)/sqrt(E)+ (%4.3f +/- %4.3f)/E chisq/dof = %3.2f",bestfit_constant,bestfit_constant_error,bestfit_coeff,bestfit_coeff_error,bestfit_add,bestfit_add_error,chisqdf));
