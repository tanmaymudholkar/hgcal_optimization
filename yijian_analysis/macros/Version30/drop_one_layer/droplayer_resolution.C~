...
void plotE_resolution(int droplayer){//main  
   ...
   const int truncationwidth=1; // truncate summation of energy in one layer
 ...
 // for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
  for (unsigned int eta_counter = 1; eta_counter<=1; eta_counter++) {
	...
	lower_bound_for_hist = 0.2*49*energy_incoming_gev;
	upper_bound_for_hist = 1.0*49*energy_incoming_gev;
      }
      else{
	lower_bound_for_hist = 0.2*49*energy_incoming_gev;
	upper_bound_for_hist = 1.0*49*energy_incoming_gev;
    ...
    myc->Update();
    myc->Print(Form("plot_calibration_fit_droplayer") +droplayer_portion+ eta_portion + Form(".png"));
    ...
    myc->Update();
    myc->Print(Form("plot_resolutions_versus_energy_droplayer_")+droplayer_portion + eta_portion + Form(".png"));                      
   }// end for eta
