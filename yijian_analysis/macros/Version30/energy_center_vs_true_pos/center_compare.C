	...
	lRecTree->GetEntry(ievt);
	  ...
	  double energy = lHit.energy();
	   ...
	   gen_posy0 = gHit.y();
        ...
        //printf("gen_x[%d]=%lf\n",i,gen_y[i]);
        TGraph *realhitx=new TGraph(layers,layer_array,gen_x);
        realhitx->SetLineColor(4);
        realhitx->SetMarkerStyle(21);
        TGraph *realhity=new TGraph(layers,layer_array,gen_y);
        realhity->SetLineColor(4);
        realhity->SetMarkerStyle(21);
        TGraph *hitcenterx=new TGraph(layers,layer_array,centerx);
        hitcenterx->SetLineColor(2);
        TGraph *hitcentery=new TGraph(layers,layer_array,centery);
        hitcentery->SetLineColor(2);
        myc->cd(1);
        hitcenterx->Draw("AC*");
        hitcenterx->SetTitle("x vs layer");
        realhitx->Draw("CP");
        myc->cd(2);
        hitcentery->Draw("AC*");
        hitcentery->SetTitle("y vs layer");
        realhity->Draw("CP");
