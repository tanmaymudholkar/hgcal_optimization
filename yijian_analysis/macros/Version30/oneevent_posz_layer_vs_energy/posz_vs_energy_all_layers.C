...
void createfilename(int eventnum, char filename[]){
  ...
  char prestring[100]="posz_vs_enengy_et50_eta2pt1_event";
  char eventnumchar[4];
  char midstring[50]="_all_layers";
  const char filefix[5]=".png";
  convert_num_to_string(eventnum,eventnumchar);
  strcat(prestring,eventnumchar);
  strcat(midstring,filefix);
  strcat(prestring,midstring);
  strcpy(filename,prestring);
}

void posz_vs_energy_all_layers_sig_event(int eventnum){
    ...
    TH2F *layer_z_E = new TH2F("","hit postion z versus energy",28,-0.5,27.5,40,3200,3600);
    ...
    for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over rechits
      ...
      layer_z_E->Fill(layer,posz,energy);
      //endif
    ...
    layer_z_E->Draw("LEGO");//draw the diagram of TH2F x_y_E // Note: if you do not need this histogram, just delete it~ 
...
void posz_vs_energy_all_layers(){//main
  int eventnum;
  for (eventnum=100;eventnum<=900;eventnum+=100){
       posz_vs_energy_all_layers_sig_event(eventnum);
  }
}
