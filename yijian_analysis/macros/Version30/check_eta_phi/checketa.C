...
void checketa(){//main  
  ...
  FILE *fp=fopen("check_eta.txt","w");
  fprintf(fp,"evt        et        eta        eta(p)     eta(f)    tanphi(p)     tanphi(f)\n");
  ...
  //for (unsigned int eta_counter = 1; eta_counter<=1; eta_counter++) {
	...
	lRecTree->GetEntry(ievt);
        ...
        double gen_posx0,gen_posy0,gen_posz0,gen_px,gen_py,gen_pz,gen_eta,gen_phi;
	   ...
	   gen_posy0 = gHit.y();
        ...
        double tanphip=gen_py/gen_px;
        double etap=-log(tan(atan(sqrt((gen_px*gen_px+gen_py*gen_py)/(gen_pz*gen_pz)))/2));
        fprintf(fp,"%d   %lf   %lf   %lf   %lf   %lf   %lf\n",ievt,et_values[et_counter],eta_values[eta_counter],etap,gen_eta,tanphip,tan(gen_phi));
      }//loop on events
