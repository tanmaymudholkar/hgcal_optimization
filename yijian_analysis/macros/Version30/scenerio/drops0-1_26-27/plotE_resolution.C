...
void plotE_resolution(){//main  
  ...
  //for (unsigned int eta_counter = 1; eta_counter<=1; eta_counter++) {
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over reconstructed hits
	  ...
	  if(layer<26 && layer>1 && sumE[layer]>0 && abs(int(posx)/10-(int)(centerx[layer])/10)<=truncationwidth && abs((int)posy/10-(int)(centery[layer])/10)<=truncationwidth) // truncate around energy center 
