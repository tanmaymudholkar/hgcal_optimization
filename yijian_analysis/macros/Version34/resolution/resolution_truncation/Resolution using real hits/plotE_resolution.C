...
void plotE_resolution(){//main  
   ...
   const int truncationwidth=0; // truncate summation of energy in one layer
  ...
  //for (unsigned int eta_counter = 1; eta_counter<=1; eta_counter++) {
	...
	lower_bound_for_hist = 0*49*energy_incoming_gev;
	...
	lRecTree->GetEntry(ievt);
        ...
        double centerz[layers]={0},sumz[layers]={0},sumE[layers]={0};
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over reconstructed hits
	  ...
	  if(sumE[layer]>0 && abs(int(posx)/10-(int)(gen_x[layer])/10)<=truncationwidth && abs((int)posy/10-(int)(gen_y[layer])/10)<=truncationwidth) // truncate around energy center 
