 ...
 // for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
  for (unsigned int eta_counter = 1; eta_counter <=1 ; eta_counter+=1){
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over rechits
	  ...
	  if(sumE[layer]>0 && abs(int(posx)/10-(int)(centerx[layer])/10)<=truncationwidth && abs((int)posy/10-(int)(centery[layer])/10)<=truncationwidth) // truncate around energy center 
    ...
    resolutions_versus_energy->SetTitle(start_title_resolutions + Form("%6.5f +/- %6.5f) + (%4.3f +/- %4.3f)/sqrt(E) chisq/dof = %3.2f",bestfit_constant,bestfit_constant_error,bestfit_coeff,bestfit_coeff_error,chisqdf));
