...
parser.add_option('-t', '--git-tag'     ,    dest='gittag'             , help='git tag version'              , default='')
...
INPATHPU = opt.pileupDirectory
nPuVtx = opt.avgPileup
...
if (opt.version==8) :
    ...
    noise='0-35:0,36-51:{opt.BHNoisemips}'.format(**locals())
    # threshold='0-51:5'
    # threshold='0-9:4.1,10-51:5'
    threshold='0-9:0.041,10-51:0.05'
...
for interCalib in interCalibList:
    if nPuVtx>0 :
        digiSuffix='%s_Pu%d_IC%d'%(opt.versionName, nPuVtx,interCalib)
        myqueue=opt.lqueue
    else :
        digiSuffix='%s_IC%d'%(opt.versionName, interCalib)
        myqueue=opt.squeue

    if opt.BHNoisemips>0: digiSuffix = "%s_bhnoise%.2fmips"%(digiSuffix, opt.BHNoisemips)

    if opt.model!=2 : digiSuffix='%s_Si%d'%(digiSuffix,nSiLayers)
    ...
    if len(opt.eos)>0:
        ...
        eosDir='%s'%(opt.eos) 
        eosDirIn='root://eoscms//eos/cms%s'%(opt.eosin) # without datatype
    else:
        eosDir='%s/'%(outDir)
        eosDirIn='%s/'%(outDir)
        
    outlog='%s/digitizer%s.log'%(outDir,digiSuffix)
    g4log='digijob%s.log'%(digiSuffix)
    os.system('mkdir -p %s'%outDir)
    
    #wrapper
    scriptFile = open('%s/runDigiJob%s.sh'%(outDir,digiSuffix), 'w')
    ...
    #submit
    os.system('chmod u+rwx %s/runDigiJob%s.sh'%(outDir,digiSuffix))
    if opt.nosubmit : os.system('echo bsub -q %s %s/runDigiJob%s.sh'%(myqueue,outDir,digiSuffix)) 
    else: os.system("bsub -q %s \'%s/runDigiJob%s.sh\'"%(myqueue,outDir,digiSuffix))
