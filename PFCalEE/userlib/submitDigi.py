...
for nPuVtx in nPuVtxlist:
    ...
    for interCalib in interCalibList:
        ...
        for en in enlist :
            ...
            outDir='%s/git_%s/version_%d/model_%d/%s/%s/thr_%s'%(opt.out,opt.gittag,opt.version,opt.model,opt.datatype,bval,opt.threshold)
            ...
            scriptFile.write('scp -i /afs/cern.ch/user/t/tmudholk/.ssh/id_dsa %s/DigiPFcal.root tmudholk@cms02.phys.cmu.edu:/export/cmss2/tmudholk/HGCal/Digi_thr%f_%s.root\n'%(outDir,opt.threshold,outTag))
