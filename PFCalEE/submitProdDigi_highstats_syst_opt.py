...
for et in enlist :
    ...
    outTag='%s_version%d_model%d_%s_rwcuf_%.1f_rwcum_%.1f_runno_%i'%(label,opt.version,opt.model,bval,opt.rwcuf,opt.rwcum,opt.runno)
    ...
    scriptFile.write('kinit tmudholk@FNAL.GOV -k -t /afs/cern.ch/user/t/tmudholk/tanmay_kerberos.keytab\n')
    scriptFile.write('rsync -ave ssh %s/HGcal_%s.root tmudholk@cmslpc-sl6.fnal.gov:/eos/uscms/store/user/tmudholk/high_stats_syst_opt/HGcal_%s.root\n'%(eosDirIn,outTag,outTag))
    ...
    scriptFile.write('rsync -ave ssh %s/DigiPFcal.root tmudholk@cmslpc-sl6.fnal.gov:/eos/uscms/store/user/tmudholk/high_stats_syst_opt/Digi_%s.root\n'%(outDir,outTag))
