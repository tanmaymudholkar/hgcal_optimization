...
// const Int_t layers_to_drop_array[] = {};
...
void plotE_resolution_drop_no_layers_modified_weights(Int_t version_number, TString version_name, TString datadir, TString outputdir) { // main
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){ // loop over rechits
	  ...
	  bool layer_to_be_dropped = false;
	...
	// }// end else condition for counting only meaningful genvecs
      ...
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(outputdir+Form("/plots/plot_")+version_name+Form("_resolutions_")+et_portion+eta_portion+Form(".png"));
