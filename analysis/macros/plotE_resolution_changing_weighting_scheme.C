...
void plotE_resolution_changing_weighting_scheme(Int_t version_number, TString version_name, TString datadir) { // main
  ...
  std::vector<Double_t> weights_x0;
  ifstream f_layer_weights_x0;
  f_layer_weights_x0.open(datadir+Form("/layer_weights_x0.dat"));
  if (f_layer_weights_x0.is_open()) {
    while (getline(f_layer_weights_x0,line)) {
      weight = std::atof(line.c_str());
      weights_x0.push_back(weight);
    }
  }
  
  // for (unsigned weight_counter = 0; weight_counter < weights.size(); weight_counter++) {
  //   std::cout << weights[weight_counter] << std::endl;
  // }
  // std::cout << std::endl;

  // for (unsigned weight_counter = 0; weight_counter < weights_x0.size(); weight_counter++) {
  //   std::cout << weights_x0[weight_counter] << std::endl;
  // }
  // std::cout << std::endl;
  ...
  // Double_t et_values_array[] = {5,50};
  // Double_t et_values_array[] = {3,5,10,20,30,40,50,60,80,100,125,175};
  // Double_t et_values_array[] = {3,5,10,20};
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    std::vector<Double_t> mean_energies_wmips_x0;
    std::vector<Double_t> mean_energies_wmips_errors_x0;
    std::vector<Double_t> sigmas_wmips_x0;
    std::vector<Double_t> sigmas_wmips_errors_x0;
    std::vector<Double_t> resolutions_x0;
    std::vector<Double_t> resolutions_errors_x0;
    Double_t resolution_x0;
    Double_t resolution_error_x0;
    ...
    for (unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
      ...
      Double_t mean_energy_wmips_x0;
      Double_t mean_energy_wmips_error_x0;
      Double_t sigma_wmips_x0;
      Double_t sigma_wmips_error_x0;
      std::vector<Double_t> total_energies_x0;
      Double_t meanE_anticipated_x0 = 0;
      Double_t sigE_anticipated_x0 = 0;
	...
	if((*genvec)[0].trackID() >= 2) continue;
	  ...
	  for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){ // loop over rechits
		...
		Double_t weighted_energy_x0 = energy*weights_x0[layer]/tanh(eta_values[eta_counter]);
		// totalE_in_layer[layer] += weighted_energy;
		totalE += weighted_energy;
		totalE_in_layer[layer] += weighted_energy;

		totalE_x0 += weighted_energy_x0;
		totalE_in_layer_x0[layer] += weighted_energy_x0;
	  ...
	  total_energies_x0.push_back(totalE_x0);
	  // meanE_anticipated += totalE/nEvts;
	  meanE_anticipated_x0 += totalE_x0;
	  // sigE_anticipated += totalE*totalE/nEvts;
	  sigE_anticipated_x0 += totalE_x0*totalE_x0;
      ...
      meanE_anticipated_x0 = meanE_anticipated_x0/nEvts_to_count;
      sigE_anticipated_x0 = sigE_anticipated_x0/nEvts_to_count;
      sigE_anticipated_x0 = sigE_anticipated_x0 - meanE_anticipated_x0*meanE_anticipated_x0;
      sigE_anticipated_x0 = sqrt(sigE_anticipated_x0);
      ...
      Double_t lower_bound_for_hist_x0;
      Double_t upper_bound_for_hist_x0;
      ...
      lower_bound_for_hist_x0 = meanE_anticipated_x0 - sigmas_down*sigE_anticipated_x0;
      upper_bound_for_hist_x0 = meanE_anticipated_x0 + sigmas_up*sigE_anticipated_x0;
	...
	p_l->Fill(total_energies[hist_filler_counter]);
      ...
      p_l->SetTitle(version_name + Form("_") + et_portion + eta_portion + Form(". chisq/dof = %3.2f",chisqdf));
      p_l->Draw();
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_")+version_name+Form("_energy_distribution_")+et_portion+eta_portion+Form(".png"));
      ...
      myc = new TCanvas("Energy Distribution","Energy",800,600);
      myc->cd();
      TH1F *p_l_x0 = new TH1F("Energy Distribution_x0", "Energies", 50, lower_bound_for_hist_x0,upper_bound_for_hist_x0);
      for(unsigned hist_filler_counter = 0; hist_filler_counter < total_energies_x0.size(); hist_filler_counter++) {
	p_l_x0->Fill(total_energies_x0[hist_filler_counter]);
      }
      p_l_x0->Fit("gaus","IME");
      TF1 *gaussian_fit_x0 = p_l_x0->GetFunction("gaus");
      mean_energy_wmips_x0 = gaussian_fit_x0->GetParameter(1);
      mean_energy_wmips_error_x0 = gaussian_fit_x0->GetParError(1);
      sigma_wmips_x0 = gaussian_fit_x0->GetParameter(2);
      sigma_wmips_error_x0 = gaussian_fit_x0->GetParError(2);
      chisqr = gaussian_fit_x0->GetChisquare();
      ndfr = gaussian_fit_x0->GetNDF();
      std::cout << "checking ... number of ds of f is " << ndfr << std::endl;
      chisqdf = chisqr/ndfr;
      // resolution = stddev/mean_energy;
      // resolution_error = resolution*((stddev_error/stddev) + (mean_energy_error/mean_energy));
      ...
      // energies_incoming_gev.push_back(energy_incoming_gev);
      // energies_incoming_gev_errors.push_back(energy_incoming_gev_error);
      mean_energies_wmips_x0.push_back(mean_energy_wmips_x0);
      mean_energies_wmips_errors_x0.push_back(mean_energy_wmips_error_x0);
      sigmas_wmips_x0.push_back(sigma_wmips_x0);
      sigmas_wmips_errors_x0.push_back(sigma_wmips_error_x0);
      // resolutions.push_back(resolution);
      // resolutions_errors.push_back(resolution_error);
      
      p_l_x0->SetTitle(version_name + Form("_") + et_portion + eta_portion + Form(". chisq/dof = %3.2f",chisqdf));
      p_l_x0->Draw();
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_")+version_name+Form("_energy_distribution_x0_")+et_portion+eta_portion+Form(".png"));
      // cout << "min layer = " << min_layer << std::endl;
      // cout << "max layer = " << max_layer << std::endl;
      delete gaussian_fit_x0;
      delete p_l_x0;
      delete myc;

      Double_t totalE_overall = 0;
      Double_t totalE_overall_x0 = 0;

      for (int layer_counter = 0; layer_counter < 24; layer_counter++) {
	totalE_overall += totalE_in_layer[layer_counter];
	totalE_overall_x0 += totalE_in_layer_x0[layer_counter];
      }

      std::cout << "totalE_overall = " << totalE_overall << std::endl;
      std::cout << "totalE_overall_x0 = " << totalE_overall_x0 << std::endl;

      for (int layer_counter = 0; layer_counter < 24; layer_counter++) {
	totalE_in_layer[layer_counter] = totalE_in_layer[layer_counter]/totalE_overall;
	totalE_in_layer_x0[layer_counter] = totalE_in_layer_x0[layer_counter]/totalE_overall_x0;
      }
      
      myc = new TCanvas("Shower Profiles","Layer",800,600);
      myc->cd();

      TH1F *p_s = new TH1F("Shower Profile", "layer", 24,-0.5,23.5);
      TH1F *p_s_x0 = new TH1F("Shower Profile_x0", "layer", 24,-0.5,23.5);

      for(unsigned layer_counter = 0; layer_counter < 24; layer_counter++) {
	p_s->Fill(layer_counter,totalE_in_layer[layer_counter]);
	p_s_x0->Fill(layer_counter,totalE_in_layer_x0[layer_counter]);
      }

      p_s->SetTitle(Form("Comparison of shower profiles at ET = %.0f",et_values[et_counter]));
      p_s_x0->SetTitle(Form("Comparison of shower profiles at ET = %.0f",et_values[et_counter]));
      p_s_x0->SetLineColor(kRed);

      p_s->SetMaximum(1.1*std::max(p_s->GetMaximum(),p_s_x0->GetMaximum()));
      p_s_x0->SetMaximum(1.1*std::max(p_s->GetMaximum(),p_s_x0->GetMaximum()));

      p_s->Draw();
      p_s_x0->Draw("same");
      myc->Update();

      myc->Print(Form("plot_")+version_name+Form("_shower_profile_comparison_")+et_portion+eta_portion+Form(".png"));
    ...
    TVectorD Tmean_energies_wmips_x0(mean_energies_wmips_x0.size(),&mean_energies_wmips_x0[0]);
    TVectorD Tmean_energies_wmips_errors_x0(mean_energies_wmips_errors_x0.size(),&mean_energies_wmips_errors_x0[0]);
    ...
    // TVectorD Tsigmas_wmips(sigmas_wmips.size(),&sigmas_wmips[0]);
    // TVectorD Tsigmas_wmips_errors(sigmas_wmips_errors.size(),&sigmas_wmips_errors[0]);
    // TVectorD Tenergies_incoming_gev_x0(energies_incoming_gev_x0.size(),&energies_incoming_gev_x0[0]);
    // TVectorD Tenergies_incoming_gev_errors_x0(energies_incoming_gev_errors_x0.size(),&energies_incoming_gev_errors_x0[0]);
    ...
    myc->Print(Form("plot_")+version_name+Form("_calibration_fit")+ eta_portion + Form(".pdf"));
    delete calibration_fit;
    delete energy_mips_calibration;
    delete myc;

    myc = new TCanvas("Energy mips calibration_x0", "mips versus gev",800,600);
    myc->cd();
    TGraphErrors *energy_mips_calibration_x0 = new TGraphErrors(Tenergies_incoming_gev,Tmean_energies_wmips_x0,Tenergies_incoming_gev_errors,Tmean_energies_wmips_errors_x0);
    energy_mips_calibration_x0->Fit("pol1");
    TF1 *calibration_fit_x0 = energy_mips_calibration_x0->GetFunction("pol1");
    chisqr = calibration_fit_x0->GetChisquare();
    ndfr = calibration_fit_x0->GetNDF();
    std::cout << "checking ... number of ds of f is " << ndfr << std::endl;
    chisqdf = chisqr/ndfr;
    Double_t offset_x0 = calibration_fit_x0->GetParameter(0);
    Double_t offset_error_x0 = calibration_fit_x0->GetParError(0);
    Double_t conversion_factor_x0 = calibration_fit_x0->GetParameter(1);
    Double_t conversion_factor_error_x0 = calibration_fit_x0->GetParError(1);
    TString start_title_conversion_factor_x0 = Form("E_mips = (");
    energy_mips_calibration_x0->SetTitle(start_title_conversion_factor_x0 + Form("%5.3f +/- %5.3f) * E_GeV + (%.1f +/- %.1f). chisq/dof = %.2f",conversion_factor_x0,conversion_factor_error_x0,offset_x0,offset_error_x0,chisqdf));
    energy_mips_calibration_x0->Draw("AP");
    myc->Update();
    // myc->Print((Form("plot_thr_")+str_threshold+Form("_v")+(Form("%i",version_number)+(Form("_calibration_fit") + eta_portion))) + Form(".pdf"));
    myc->Print(Form("plot_")+version_name+Form("_calibration_fit_x0")+ eta_portion + Form(".pdf"));
    delete calibration_fit_x0;
    delete energy_mips_calibration_x0;
    delete myc;
    ...
    for (unsigned int sigmas_wmips_counter = 0; sigmas_wmips_counter != sigmas_wmips.size(); sigmas_wmips_counter++) {
      ...
      resolution_x0 = sigmas_wmips_x0[sigmas_wmips_counter]/(mean_energies_wmips_x0[sigmas_wmips_counter]-offset_x0);
      resolution_error_x0 = resolution_x0*sqrt(pow(sigmas_wmips_errors_x0[sigmas_wmips_counter]/sigmas_wmips_x0[sigmas_wmips_counter],2)+(pow(mean_energies_wmips_errors_x0[sigmas_wmips_counter],2)+pow(offset_error_x0,2))/pow(mean_energies_wmips_x0[sigmas_wmips_counter]-offset_x0,2));
      resolutions_x0.push_back(resolution_x0);
      resolutions_errors_x0.push_back(resolution_error_x0);
    }
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("data_")+eta_portion+Form("_");
    ...
    TVectorD Tresolutions_x0(resolutions_x0.size(),&resolutions_x0[0]);
    TVectorD Tresolutions_errors_x0(resolutions_errors_x0.size(),&resolutions_errors_x0[0]);
    ofstream outfile_x0;
    TString outfile_name_prefix_x0 = Form("data_")+eta_portion+Form("_");
    outfile_x0.open(outfile_name_prefix_x0+version_name+Form("_resolutions_x0"));
    for(unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
      outfile_x0 << energies_incoming_gev[et_counter] << "   " << energies_incoming_gev_errors[et_counter] << "   " << resolutions_x0[et_counter] << "   " << resolutions_errors_x0[et_counter] << std::endl;
    }
    outfile_x0.close();
    ...
    myc->Update();
    myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_standalone_") + eta_portion))) + Form(".pdf"));
    delete resolutions_versus_energy_standalone;
    delete function_to_fit_standalone;
    delete myc;
    std::cout << std::endl;

    myc = new TCanvas("Resolutions versus Energy_standalone_x0","Resolution versus Energy",800,600);
    myc->cd();
    TGraphErrors *resolutions_versus_energy_standalone_x0 = new TGraphErrors(Tenergies_incoming_gev,Tresolutions_x0,Tenergies_incoming_gev_errors,Tresolutions_errors_x0);
    TF1 *function_to_fit_standalone_x0 = new TF1("f_to_fit_standalone_x0",resolutions_fit_standalone,0.0001,1300,2);
    function_to_fit_standalone_x0->SetParameters(0.25,0.01);
    // function_to_fit_standalone->SetParLimits(2,0.0,0.2);
    function_to_fit_standalone_x0->SetParNames("stoch_term","const_term");
    resolutions_versus_energy_standalone_x0->Fit(function_to_fit_standalone_x0,"IME");
    chisqr = function_to_fit_standalone_x0->GetChisquare();
    ndfr = function_to_fit_standalone_x0->GetNDF();
    std::cout << "checking ... number of ds of f is " << ndfr << std::endl;
    chisqdf = chisqr/ndfr;
    Double_t fit_stoch_x0 = function_to_fit_standalone_x0->GetParameter(0);
    Double_t fit_stoch_error_x0 = function_to_fit_standalone_x0->GetParError(0);
    Double_t fit_const_x0 = function_to_fit_standalone_x0->GetParameter(1);
    Double_t fit_const_error_x0 = function_to_fit_standalone_x0->GetParError(1);
    // fit_stoch = function_to_fit_standalone->GetParameter(0);
    // fit_stoch_error = function_to_fit_standalone->GetParError(0);
    // fit_const = function_to_fit_standalone->GetParameter(1);
    // fit_const_error = function_to_fit_standalone->GetParError(1);
    TString start_title_resolutions_x0 = version_name + eta_portion + Form(": res = (");
    // start_title_resolutions = version_name + eta_portion + Form(": res = (");
    resolutions_versus_energy_standalone_x0->SetTitle(start_title_resolutions + Form("%6.5f +/- %6.5f) + (%4.3f +/- %4.3f)/sqrt(E/GeV). chisq/dof = %.1f",fit_const_x0,fit_const_error_x0,fit_stoch_x0,fit_stoch_error_x0,chisqdf));
    resolutions_versus_energy_standalone_x0->Draw("AP");
    myc->Update();
    myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_standalone_x0_") + eta_portion))) + Form(".pdf"));
    delete resolutions_versus_energy_standalone_x0;
    delete function_to_fit_standalone_x0;
    delete myc;
    std::cout << std::endl;
    ...
    // myc->cd();
    // TGraphErrors *resolutions_versus_energy_standalone = new TGraphErrors(Tenergies_incoming_gev,Tresolutions,Tenergies_incoming_gev_errors,Tresolutions_errors);
    // TF1 *function_to_fit_standalone = new TF1("f_to_fit_standalone",resolutions_fit_standalone,0.0001,1300,3);
    // function_to_fit_standalone->SetParameters(0.25,0.01,0.00);
    // function_to_fit_standalone->SetParLimits(2,0.0,0.2);
    // function_to_fit_standalone->SetParNames("stoch_term","const_term","noise_term");
    // resolutions_versus_energy_standalone->Fit(function_to_fit_standalone,"IME");
    // chisqr = function_to_fit_standalone->GetChisquare();
    // ndfr = function_to_fit_standalone->GetNDF();
    // std::cout << "checking ... number of ds of f is " << ndfr << std::endl;
    ...
    // TString start_title_resolutions = version_name + eta_portion + Form(": res = (");
    // // start_title_resolutions = version_name + eta_portion + Form(": res = (");
    // resolutions_versus_energy_standalone->SetTitle(start_title_resolutions + Form("%6.5f +/- %6.5f) + (%4.3f +/- %4.3f)/sqrt(E/GeV) + (%4.3f +/- %4.3f)/(E/GeV). chisq/dof = %.1f",fit_const,fit_const_error,fit_stoch,fit_stoch_error,fit_noise,fit_noise_error,chisqdf));
    // resolutions_versus_energy_standalone->Draw("AP");
    // myc->Update();
    // myc->Print((Form("plot_standard_")+(version_name+(Form("_resolutions_versus_energy_standalone_withnoise_") + eta_portion))) + Form(".pdf"));
    // delete resolutions_versus_energy_standalone;
    // delete function_to_fit_standalone;
    // delete myc;
    // std::cout << std::endl;
