...
void plotE_resolution_drop_layers(Int_t version_number, TString version_name, TString datadir) { // main
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    for (unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
      ...
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_")+version_name+Form("_resolutions_")+et_portion+eta_portion+Form(".png"));
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("data")+eta_portion+Form("_");
    ...
    myc->Update();
    myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_standalone_withnoise_") + eta_portion))) + Form(".pdf"));
