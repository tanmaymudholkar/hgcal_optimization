...
Double_t resolutions_fit(Double_t *x,Double_t *par) {
  //return (0.009 + par[0]/sqrt(x[0]) + par[1]/x[0]);
  return (par[0] + par[1]/sqrt(x[0]));
}

void plotE_resolution_trunc(){//main  
  const int layers=24;
 ...
 // for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
  for (unsigned int eta_counter = 1; eta_counter <=1 ; eta_counter+=1){
    ...
    for (unsigned int et_counter = 0; et_counter < et_values.size(); et_counter+=1) {
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over rechits
	  ...
	  //if(sumE[layer]>0 && abs((int)((posx-centerx[layer])/10))<=truncationwidth && abs((int)((posy-centery[layer])/10))<=truncationwidth) // truncate around energy center
	  if(sumE[layer]>0 && (((posx-centerx[layer])/10)*((posx-centerx[layer])/10)+((posy-centery[layer])/10)*((posy-centery[layer])/10))<truncationwidth*truncationwidth) {
    ...
    //function_to_fit->SetParameters(0.025,-0.5);
    function_to_fit->SetParameters(0.02,0.23);
    function_to_fit->SetParNames("constant_term","coeff_sqrt");
    ...
    double bestfit_constant_term = function_to_fit->GetParameter(0);
    double bestfit_constant_term_error = function_to_fit->GetParError(0);

    // double bestfit_coeff = function_to_fit->GetParameter(0);
    // double bestfit_coeff_error = function_to_fit->GetParError(0);
    // double bestfit_coeff_inv = function_to_fit->GetParameter(1);
    // double bestfit_coeff_inv_error = function_to_fit->GetParError(1);
    
    resolutions_versus_energy->SetTitle(start_title_resolutions + Form("(%5.4f +/- %5.4f) + (%4.3f +/- %4.3f)/sqrt(E); chisq/dof = %3.2f",bestfit_constant_term,bestfit_constant_term_error,bestfit_coeff,bestfit_coeff_error,chisqdf));
    //resolutions_versus_energy->SetTitle(start_title_resolutions + Form("0.009 + (%4.3f +/- %4.3f)/sqrt(E) +/- (%4.3f +/- %4.3f)/E); chisq/dof = %3.2f",bestfit_coeff,bestfit_coeff_error,bestfit_coeff_inv,bestfit_coeff_inv_error,chisqdf));
