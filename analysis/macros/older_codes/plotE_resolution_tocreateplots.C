...
void plotE_resolution_tocreateplots(Int_t version_number, TString version_name, TString datadir, Double_t threshold, Double_t sigmas_down, Double_t sigmas_up, Int_t signal_region) { // main
  // void plotE_resolution(Int_t version_number, TString version_name, TString datadir, Double_t threshold, Double_t sigmas_down, Double_t sigmas_up){// main
  // int main(Int_t version_number, TString version_name, TString datadir, Double_t threshold, Double_t sigmas_down, Double_t sigmas_up){// main
  ...
  // std::string inFilePath = "export_dir/version34/HGcal__version34_model2_BOFF_et20_eta2.100.root";
  ...
  //   std::cout << "layer " << layer_counter << " : cellsize is " << geomConv.cellSize(layer_counter,700.1) << std::endl;
  // }
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    Double_t totalE_across_layers = 0;
    for (int layer_counter = 0; layer_counter < 24; layer_counter++) {
      totalE_across_layers += totalE_in_layer[layer_counter];
    }

    for (unsigned layer_counter = 0; layer_counter < 24; layer_counter++) {
      totalE_in_layer[layer_counter] = totalE_in_layer[layer_counter]/(nEvts_to_count*totalE_across_layers);
    }

    // std::cout << "mean_anticipated is " << meanE_anticipated << std::endl;
    // std::cout << "sigma_anticipated is " << sigE_anticipated << std::endl;
    et=175;
    energy_incoming_gev = et*cosh(eta_values[eta_counter]);
    et_portion = Form("et%.0f",et);
          
    TChain  *lSimTree175 = new TChain("HGCSSTree");
    TChain  *lRecTree175 = new TChain("RecoTree");

    lSimTree175->AddFile(HGcal_common_prefix+et_portion+eta_portion+common_suffix);
    lRecTree175->AddFile(Digi_common_prefix+et_portion+eta_portion+common_suffix);

    std::vector<HGCSSSamplingSection> * ssvec175 = 0;
    std::vector<HGCSSSimHit> * simhitvec175 = 0;
    std::vector<HGCSSRecoHit> * rechitvec175 = 0;
    std::vector<HGCSSGenParticle> * genvec175 = 0; // get the generated particle vector
      
    lSimTree175->SetBranchAddress("HGCSSSamplingSectionVec",&ssvec175);
    lSimTree175->SetBranchAddress("HGCSSSimHitVec",&simhitvec175);
    lSimTree175->SetBranchAddress("HGCSSGenParticleVec",&genvec175);
    lRecTree175->SetBranchAddress("HGCSSRecoHitVec",&rechitvec175);
      
    const unsigned nEvts175 = lSimTree175->GetEntries();
    unsigned nEvts_to_count175 = 0;
    Double_t totalE175(0);
    Double_t totalE_in_layer175[24];

    for (unsigned layer_counter = 0; layer_counter < 24; layer_counter++) {
      totalE_in_layer175[layer_counter] = 0;
    }
      
    for (unsigned ievt(0); ievt<nEvts175; ++ievt){// loop on entries
      totalE175 = 0;
		
      if (ievt%100==0) std::cout << " -- Processing event " << ievt << std::endl;
      lSimTree175->GetEntry(ievt);
      lRecTree175->GetEntry(ievt);

      //std::cout << "track id " << (*genvec)[0].trackID() << "  genvec size " << (*genvec).size() << std::endl;

      // if((*genvec).size() > 1) continue;  // Delete events which has converted photons.
      if((*genvec175)[0].trackID() >= 2) continue;
      else{
	nEvts_to_count175 += 1;
	const HGCSSGenParticle gHit175 = (*genvec)[0];
	//std::cout << "ACCEPTED: " << "track id " << (*genvec)[0].trackID() << "  genvec size " << (*genvec).size() << std::endl;
	Double_t posx_gen_ini175 = gHit175.x();
	Double_t posy_gen_ini175 = gHit175.y();
	Double_t posz_gen_ini175 = gHit175.z();
	Double_t px_gen175 = gHit175.px();
	Double_t py_gen175 = gHit175.py();
	Double_t pz_gen175 = gHit175.pz();
	
	// for (unsigned iH(0); iH<(*simhitvec).size(); ++iH){// loop over simhits
	for (unsigned iH(0); iH<(*rechitvec175).size(); ++iH){ // loop over rechits
	  // const HGCSSSimHit lHit = (*simhitvec)[iH];
	  const HGCSSRecoHit lHit175 = (*rechitvec175)[iH];
	  
	  Double_t posx175 = lHit175.get_x();
	  Double_t posy175 = lHit175.get_y();
	  Double_t posz175 = lHit175.get_z();
	  unsigned layer175 = lHit175.layer();
	  Double_t energy175 = lHit175.energy();
	  // std::cout << "energy is " << energy << "   " << "layer is " << layer << std::endl;
	  
	  if(energy175>threshold) {
	    Double_t posx_gen175 = posx_gen_ini175 + (px_gen175/pz_gen175)*(posz175 - posz_gen_ini175);
	    Double_t posy_gen175 = posy_gen_ini175 + (py_gen175/pz_gen175)*(posz175 - posz_gen_ini175);
	    Double_t dx175 = posx175 - posx_gen175;
	    Double_t dy175 = posy175 - posy_gen175;
	    Double_t halfCell175 = 0.5*cellsizeat(posx175,posy175);
	    if ((fabs(dx175) <= signal_region*halfCell175) && (fabs(dy175) <= signal_region*halfCell175)){
	      Double_t weighted_energy175 = energy175*weights[layer175]/tanh(eta_values[eta_counter]);
	      totalE_in_layer175[layer175] += weighted_energy175;
	      totalE175 += weighted_energy175;
	    }
	  }
	}
      }
    ...
    Double_t totalE_across_layers175 = 0;
    for (int layer_counter = 0; layer_counter < 24; layer_counter++) {
      totalE_across_layers175 += totalE_in_layer175[layer_counter];
    }
    
    for (unsigned layer_counter = 0; layer_counter < 24; layer_counter++) {
      totalE_in_layer175[layer_counter] = totalE_in_layer175[layer_counter]/(nEvts_to_count175*totalE_across_layers175);
    }

    // std::cout << "mean_anticipated is " << meanE_anticipated << std::endl;
    // std::cout << "sigma_anticipated is " << sigE_anticipated << std::endl;

    TCanvas *myc1 = new TCanvas("shower profile","layer",800,600);
    myc1->cd();
    TH1F *p_shower = new TH1F("shower profile", "layers",24,-0.5,23.5);
    TH1F *p_shower175 = new TH1F("shower profile 175", "layers",24,-0.5,23.5);
    for (unsigned layer_counter = 0; layer_counter < 24; layer_counter++) {
      p_shower->Fill(layer_counter,totalE_in_layer[layer_counter]);
      p_shower175->Fill(layer_counter,totalE_in_layer175[layer_counter]);
    }

    p_shower->SetTitle(Form("Sample Shower Profiles"));
    p_shower->Draw();
    p_shower->GetXaxis()->SetTitle("layer");
    p_shower->GetYaxis()->SetTitle("energy fraction");
    p_shower175->SetLineColor(kRed);
    p_shower175->Draw("same");
    myc1->Print(Form("shower_profiles.pdf"));
    ...
    // // p_l_unweighted->Draw();
    // // p_E->Draw();
    // myc->Print(Form("plot_")+version_name+Form("_threshold_%.1f_resolutions_",threshold)+et_portion+eta_portion+Form("_down%.1f_up%.1f_sigreg%i.png",sigmas_down,sigmas_up,signal_region));
    ...
    // myc->Print(Form("plot_")+version_name+Form("_threshold_%.1f_calibration_fit",threshold)+ eta_portion + Form("_down%.1f_up%.1f_sigreg%i.pdf",sigmas_down,sigmas_up,signal_region));
    // delete calibration_fit;
    // delete energy_mips_calibration;
    // delete myc;
    // // myc = new TCanvas("sigma mips calibration", "mips versus gev",800,600);
    // // myc->cd();
    // // TGraphErrors *sigma_mips_calibration = new TGraphErrors(Tenergies_incoming_gev,Tsigmas_wmips,Tenergies_incoming_gev_errors,Tsigmas_wmips_errors);
    // // sigma_mips_calibration->Fit("pol1");
    // // TF1 *calibration_sigma_fit = sigma_mips_calibration->GetFunction("pol1");
    // // chisqr = calibration_sigma_fit->GetChisquare();
    // // ndfr = calibration_sigma_fit->GetNDF();
    // // std::cout << "checking ... number of ds of f is " << ndfr << std::endl;
    // // chisqdf = chisqr/ndfr;
    // // Double_t offset_sigma = calibration_sigma_fit->GetParameter(0);
    // // Double_t offset_sigma_error = calibration_sigma_fit->GetParError(0);
    // // Double_t conversion_factor_sigma = calibration_sigma_fit->GetParameter(1);
    // // Double_t conversion_factor_sigma_error = calibration_sigma_fit->GetParError(1);
    // // start_title_conversion_factor = Form("sigma_mips = (");
    // // sigma_mips_calibration->SetTitle(start_title_conversion_factor + Form("%5.3f +/- %5.3f) * E_GeV + (%.1f +/- %.1f). chisq/dof = %.2f",conversion_factor_sigma,conversion_factor_sigma_error,offset_sigma,offset_sigma_error,chisqdf));
    // // sigma_mips_calibration->Draw("AP");
    // // myc->Update();
    // // // myc->Print((Form("plot_thr_")+str_threshold+Form("_v")+(Form("%i",version_number)+(Form("_calibration_sigma_fit") + eta_portion))) + Form(".pdf"));
    // // myc->Print(Form("plot_")+version_name+Form("_threshold_%.1f_calibration_sigma_fit",threshold)+ eta_portion + Form("_down%.1f_up%.1f.pdf",sigmas_down,sigmas_up));
    // // delete calibration_sigma_fit;
    // // delete sigma_mips_calibration;
    // // delete myc;
    ...
    // ofstream outfile;
    // TString outfile_name_prefix = Form("data_compare_with_earlier")+eta_portion+Form("_v");
    // // TString outfile_name_prefix = Form("data_eta_")+eta_portion+Form("_");
    // outfile.open(outfile_name_prefix+version_name+Form("_threshold_%.1f_down%.1f_up%.1f_sigreg%i_resolutions",threshold,sigmas_down,sigmas_up,signal_region));
    // for(unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
    ...
    // // myc->Update();
    // // myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_thr%.1f",threshold) + eta_portion))) + Form("_down%.1f_up%.1f_sigreg%i.pdf",sigmas_down,sigmas_up,signal_region));
    ...
    // myc->Update();
    // myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_standalone_withnoise_thr%.1f",threshold) + eta_portion))) + Form("_down%.1f_up%.1f_sigreg%i.pdf",sigmas_down,sigmas_up,signal_region));
