...
void plotE_resolution_old(Int_t version_number, TString datadir){//main
  ...
  double et_values_array[] = {3,5,10,20,30,40,50,60,80,100,125,175};
  //double et_values_array[] = {50};
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	...
	p_l->Fill(total_energies[hist_filler_counter]);
      ...
      //p_l_unweighted->Draw();
      //p_E->Draw();
      myc->SaveAs((Form("old_plot_v")+(Form("%i",version_number)+(Form("_resolutions_")+et_portion)))+eta_portion+Form(".png"));
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("old_data_eta_")+eta_portion+Form("_v");
    outfile.open(outfile_name_prefix+Form("%i",version_number)+Form("_resolutions"));
    for(unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
    ...
    myc->Update();
    myc->Print((Form("old_plot_v")+(Form("%i",version_number)+(Form("_calibration_fit") + eta_portion))) + Form(".pdf"));
    ...
    myc->Update();
    myc->Print((Form("old_plot_v")+(Form("%i",version_number)+(Form("_resolutions_versus_energy") + eta_portion))) + Form(".pdf"));
