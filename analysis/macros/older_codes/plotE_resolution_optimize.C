...
void plotE_resolution_optimize(){//main  
  ...
  f_layer_weights.open("data_v34/layer_weights_v34.dat");
  std::string line;
  double weight;
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    for (unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
      ...
      //      double totalE(0);
      //TH1F *p_l_weighted = new TH1F("Layers weighted","layer",24,-0.5,23.5);
      //TH2F *p_E_l = new TH2F("total E and layer number in ECAL","layer number",500,0,1500,24,-0.5,27.5);
	...
	// std::vector<Double_t> weighted_x[24];
	// std::vector<Double_t> weighted_y[24];
	//Double_t totalE_in_layer[24];
	//for(unsigned layer_counter = 0; layer_counter != 24; layer_counter++) {
	//    totalE_in_layer[layer_counter] = 0;
	//}
	...
	for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop over rechits
	  ...
	  totalE_across_layers += weighted_energy;
	...
	//sigE_anticipated += totalE*totalE/nEvts;
      ...
      // Double_t inv_E_factor[24];
      // Double_t inv_E_factor_total = 0;
      // Double_t inv_E_factor_experimental[24];
      // Double_t inv_E_factor_experimental_total = 0;
      // std::cout << "totalE_across_layers = " << totalE_across_layers << std::endl;
      // for (unsigned int layer_counter = 0; layer_counter != 24; layer_counter++) {
      // 	totalE_in_layer[layer_counter] = totalE_in_layer[layer_counter]/totalE_across_layers;
      // 	inv_E_factor[layer_counter] = 1/totalE_in_layer[layer_counter];
      // 	inv_E_factor_total += inv_E_factor[layer_counter];
      // 	inv_E_factor_experimental[layer_counter] = cbrt(1/totalE_in_layer[layer_counter]);
      // 	inv_E_factor_experimental_total += inv_E_factor_experimental[layer_counter];
      // }
      for (unsigned int layer_counter = 0; layer_counter != 24; layer_counter++) {
	//inv_E_factor[layer_counter] = inv_E_factor[layer_counter]/inv_E_factor_total;
	//inv_E_factor_experimental[layer_counter] = inv_E_factor_experimental[layer_counter]/inv_E_factor_experimental_total;
	//std::cout << layer_counter << "   " << inv_E_factor[layer_counter] << "   " << inv_E_factor_experimental[layer_counter] << std::endl;
	//std::cout << totalE_in_layer[layer_counter] << std::endl;
	std::printf("%f\n",totalE_in_layer[layer_counter]);
	//std::cout << inv_E_factor_experimental[layer_counter] << std::endl;
      }
      
      //      std::cout << "check: totalE = " << total_check << std::endl;
