...
void plotE_radius(){//main  
  ...
  for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
    ...
    for(unsigned layer_counter = 0; layer_counter != 28; layer_counter++) {
      if(totalE_in_layer[layer_counter] > 0) {
	//n_evts_in_layer[layer_counter]++;
	for(unsigned event_counter = 0; event_counter != weighted_x[layer_counter].size(); event_counter++) {
	  weighted_x[layer_counter][event_counter] = weighted_x[layer_counter][event_counter]/totalE_in_layer[layer_counter];
	  weighted_y[layer_counter][event_counter] = weighted_y[layer_counter][event_counter]/totalE_in_layer[layer_counter];
	}
	TVectorD Tweighted_x(weighted_x[layer_counter].size(),&weighted_x[layer_counter][0]);
	TVectorD Tweighted_y(weighted_y[layer_counter].size(),&weighted_y[layer_counter][0]);
      
	TGraph *weighted_positions = new TGraph(Tweighted_x,Tweighted_y);
	xrms[layer_counter] += weighted_positions->GetRMS(1);
	yrms[layer_counter] += weighted_positions->GetRMS(2);
	how_many_to_count[layer_counter]++;
      }
    }
    //p_l->Fill(totalE);
  }//loop on hits
  ...
  for(unsigned layer_counter = 0; layer_counter != 28; layer_counter++) {
    xrms[layer_counter] = xrms[layer_counter]/how_many_to_count[layer_counter];
    yrms[layer_counter] = yrms[layer_counter]/how_many_to_count[layer_counter];
    radius[layer_counter] = (xrms[layer_counter] + yrms[layer_counter])*0.5;
  ...
  TGraph *radii = new TGraph(28,layer_number,radius);
  radii->Draw();
  myc->Update();
  myc->Print("test_radii.pdf");
