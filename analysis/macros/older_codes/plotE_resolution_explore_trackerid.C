...
void plotE_resolution_explore_trackerid(Int_t version_number, TString version_name, TString datadir, Double_t threshold, Double_t sigmas_down, Double_t sigmas_up, Int_t signal_region) { // main
...
// int main(Int_t version_number, TString version_name, TString datadir, Double_t threshold, Double_t sigmas_down, Double_t sigmas_up){// main
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	...
	if((*genvec).size() > 1) {
	  for (unsigned genvec_counter = 0; genvec_counter < (*genvec).size(); ++genvec_counter) {
	    std::cout << std::setprecision(6)
		<< "====================================" << std::endl
		      << " = time " << ((*genvec)[genvec_counter]).time() << " ns" << std::endl
		      << " = position " << ((*genvec)[genvec_counter]).x() << " " << ((*genvec)[genvec_counter]).y() << " " << ((*genvec)[genvec_counter]).z()<< " mm" << std::endl
		      << " = Mass " << ((*genvec)[genvec_counter]).mass()<< " MeV" << std::endl
		      << " = momentum " << ((*genvec)[genvec_counter]).px() << " " <<  ((*genvec)[genvec_counter]).py() << " " << ((*genvec)[genvec_counter]).pz()<< " MeV" << std::endl
		      << " = pdgid " << ((*genvec)[genvec_counter]).pdgid()<< std::endl
		      << " = charge " << ((*genvec)[genvec_counter]).charge()<< std::endl
		      << " = G4trackID " << ((*genvec)[genvec_counter]).trackID()<< std::endl
		      << "====================================" << std::endl;
	  }
	  continue;
	}// Delete events which has converted photons.
	else{
	  nEvts_to_count += 1;
    ...
    // myc->Update();
    // myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy") + eta_portion))) + Form("_down%.1f_up%.1f_sigreg%i.pdf",sigmas_down,sigmas_up,signal_region));
    ...
    myc->Update();
    myc->Print((Form("plot_")+(version_name+(Form("_resolutions_versus_energy_standalone") + eta_portion))) + Form("_down%.1f_up%.1f_sigreg%i.pdf",sigmas_down,sigmas_up,signal_region));
