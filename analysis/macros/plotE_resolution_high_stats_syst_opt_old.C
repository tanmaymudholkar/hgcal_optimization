...
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSEvent.hh"
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSInfo.hh"
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSRecoHit.hh"
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSSimHit.hh"
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSSamplingSection.hh"
#include "/home/tmudholk/research/hgcal_analysis/hgcal_optimization/userlib/include/HGCSSGenParticle.hh"
...
void plotE_resolution_high_stats_syst_opt(Int_t version_number, TString version_name, TString datadir, TString outputdir, Double_t et, Double_t rwcuf, Double_t rwcum) { // main
  ...
  ofstream outfile;
  TString outfile_name_prefix = outputdir+Form("/resolutions/data_resolution_et%.0f_rwcuf%.1f_rwcum%.1f",et,rwcuf,rwcum);
  // outfile.open(outfile_name_prefix+version_name);
  outfile.open(outfile_name_prefix);
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    unsigned run_no = 1;
    //for (unsigned run_no = 1; run_no <= 61; run_no++) {
    ...
    while (consecutive_nonexistent_files < 4) {
      ...
      if (hgcal_data_exists && digi_data_exists) {
	...
	if (cellSize != 2.5) {
	  std::cout << "PROBLEM: Nonstandard cellSize not supported!" << std::endl;
	  std::exit(EXIT_FAILURE);
	}
    ...
    for (unsigned ievt(0); ievt<nEvts; ++ievt){// begin loop on events
      totalE = 0;
      ...
      if((*genvec)[0].trackID() >= 2) continue;
    	...
    	// meanE_statistical += totalE/nEvts;
    	meanE_statistical += totalE;
    	// sigE_statistical += totalE*totalE/nEvts;
