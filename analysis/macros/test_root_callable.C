...
void test_root_callable() { // main
  ...
  Double_t ndfr;
  Double_t chisqdf;
  Int_t version_number=45;
  Double_t et=150;
  // TString datadir("root://eoscms.cern.ch//eos/cms/store/user/tmudholk/high_stats_syst_opt/gitV05-02-04/gamma");
  TString datadir("/afs/cern.ch/user/t/tmudholk/private/mount/eos_cern/cms/store/user/tmudholk/high_stats_syst_opt/gitV05-02-04/gamma");
  ...
  for (Double_t rwcuf = 0; rwcuf<=1.0; rwcuf += 0.1) {
	...
	for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	  ...
	  for (unsigned run_no = 1; run_no <= 31; run_no++) {
	    ...
	    // std::cout << (HGcal_common_prefix+Form("runno_%i_",run_no)+et_portion+eta_portion+common_suffix) << std::endl;
	    // std::cout << "_______________________________________________________" << std::endl;
	    
	  
	    // TFile *inputFile = TFile::Open(HGcal_common_prefix+Form("runno_%i_",run_no)+et_portion+eta_portion+common_suffix);
	    // if ((*inputFile).IsZombie()) {
	    //   std::cout << "The following file does not exist:" << std::endl;
	    //   std::cout << (HGcal_common_prefix+Form("runno_%i_",run_no)+et_portion+eta_portion+common_suffix) << std::endl;
	    // }
	    TFile *testFile;
	    bool isOK=testInputFile(HGcal_common_prefix+Form("runno_%i_",run_no)+et_portion+eta_portion+common_suffix, testFile);
