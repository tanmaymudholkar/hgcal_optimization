...
const Double_t calibration_offset = 15.4;
const Double_t calibration_offset_error = 1.1;
const Double_t calibration_slope = 43.708;
const Double_t calibration_slope_error = 0.012;
...
void plotE_resolution_high_stats(Int_t version_number, TString version_name, TString datadir) { // main
  ...
  Double_t et_values_array[] = {3,20,50,100};
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("data_high_stats")+eta_portion+Form("_");
    ...
    for (unsigned int et_counter = 0; et_counter != et_values.size(); et_counter++) {
      ...
      for (unsigned run_no = 1; run_no <= 100; run_no++) {
	...
	p_l->Fill(total_energies[hist_filler_counter]);
      ...
      resolution = sigma_wmips/(mean_energy_wmips-calibration_offset);
      resolution_error = resolution*sqrt(pow(sigma_wmips_error/sigma_wmips,2)+(pow(mean_energy_wmips_error,2)+pow(calibration_offset_error,2))/pow(mean_energy_wmips-calibration_offset,2));
      // resolutions.push_back(resolution);
      // resolutions_errors.push_back(resolution_error);
      outfile << energy_incoming_gev << "   " << energy_incoming_gev_error << "   " << resolution << "   " << resolution_error << std::endl;
      
      p_l->SetTitle(version_name + Form("_") + et_portion + eta_portion + Form(". chisq/dof = %3.2f",chisqdf) + Form("_threshold=%.1f",threshold));
      p_l->Draw();
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_high_stats_")+version_name+Form("_resolutions_")+et_portion+eta_portion+Form(".png"));
    ...
    // myc->Print(Form("plot_standard_")+version_name+Form("_calibration_fit")+ eta_portion + Form(".pdf"));
    ...
    //   resolution = sigmas_wmips[sigmas_wmips_counter]/(mean_energies_wmips[sigmas_wmips_counter]-calibration_offset);
    //   resolution_error = resolution*sqrt(pow(sigmas_wmips_errors[sigmas_wmips_counter]/sigmas_wmips[sigmas_wmips_counter],2)+(pow(mean_energies_wmips_errors[sigmas_wmips_counter],2)+pow(calibration_offset_error,2))/pow(mean_energies_wmips[sigmas_wmips_counter]-calibration_offset,2));
    //   resolutions.push_back(resolution);
    //   resolutions_errors.push_back(resolution_error);
    // }
