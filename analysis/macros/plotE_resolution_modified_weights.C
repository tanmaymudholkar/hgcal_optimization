...
void plotE_resolution_modified_weights(Int_t version_number, TString version_name, TString datadir) { // main
  ...
  std::vector<Double_t> unmodified_weights;
  ...
  if (f_layer_weights.is_open()) {
    while (getline(f_layer_weights,line)) {
      ...
      unmodified_weights.push_back(weight);
    }
  }

  std::vector<Double_t> weights;
  weights.push_back(unmodified_weights[0]+0.5*unmodified_weights[1]);
  for (unsigned int layer_counter = 1; layer_counter < (unmodified_weights.size() - 1); layer_counter++) {
    weights.push_back(0.5*(unmodified_weights[layer_counter]+unmodified_weights[layer_counter+1]));
  }
  weights.push_back(0.5*unmodified_weights[(unmodified_weights.size()-1)]);

  for (unsigned int layer_counter = 0; layer_counter < weights.size(); layer_counter++) {
    std::cout << weights[layer_counter] << std::endl;
  }
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	...
	p_l->Fill(total_energies[hist_filler_counter]);
      ...
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_modified_weights_")+version_name+Form("_resolutions_")+et_portion+eta_portion+Form(".png"));
    ...
    myc->Print(Form("plot_modified_weights_")+version_name+Form("_calibration_fit")+ eta_portion + Form(".pdf"));
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("data_modified_weights")+eta_portion+Form("_");
    ...
    myc->Update();
    myc->Print((Form("plot_modified_weights_")+(version_name+(Form("_resolutions_versus_energy_standalone_withnoise_") + eta_portion))) + Form(".pdf"));
    ...
    myc->Update();
    myc->Print((Form("plot_modified_weights_")+(version_name+(Form("_resolutions_versus_energy_standalone_") + eta_portion))) + Form(".pdf"));
