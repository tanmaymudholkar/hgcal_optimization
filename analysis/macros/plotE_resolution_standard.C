...
void plotE_resolution_standard(Int_t version_number, TString version_name, TString datadir) { // main
  ...
  for (unsigned int eta_counter = 0; eta_counter != eta_values.size(); eta_counter++) {
	...
	p_l->Fill(total_energies[hist_filler_counter]);
      ...
      // p_l_unweighted->Draw();
      // p_E->Draw();
      myc->Print(Form("plot_standard_")+version_name+Form("_resolutions_")+et_portion+eta_portion+Form(".png"));
    ...
    ofstream outfile;
    TString outfile_name_prefix = Form("data_standard")+eta_portion+Form("_");
    ...
    myc->Update();
    myc->Print((Form("plot_standard_")+(version_name+(Form("_resolutions_versus_energy_standalone_") + eta_portion))) + Form(".pdf"));
